...
@section('title', 'For Approval | EngiStart')
@section('header', 'For Approval')
@section('subheader', 'Review Division Head recommendations and issue VP approval decisions.')
...
<div class="p-6 overflow-y-auto h-full">
    <div class="w-full">
        <style>
            ...
            .apis-remarks-control::placeholder {
                color: var(--text3);
            }
        ...
        @if ($this->filteredInboxItems->isNotEmpty())
            @include('partials.apis.alert', [
                'type' => 'warn',
                'message' => $this->filteredInboxItems->count() . ' request' . ($this->filteredInboxItems->count() !== 1 ? 's' : '') . ' pending your approval.',
            ])
        @endif

        <div class="rounded-[12px] p-[12px_14px] mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"
             style="border: 0.5px solid var(--border); background: var(--bg2)">
            <div class="grid grid-cols-1 md:grid-cols-[minmax(0,1.5fr)_180px_180px] gap-3 flex-1 w-full">
                <div>
                    ...
                    <input type="text" wire:model.live.debounce.300ms="search" class="apis-toolbar-control" placeholder="Search by request ID, title, farm, or requester...">
                </div>
                <div>
                    <label class="block text-[10px] text-apis-text2 mb-1 font-medium uppercase tracking-[0.07em]">Type</label>
                    <select wire:model.live="typeFilter" class="apis-toolbar-control">
                ...
                <div>
                    ...
                    <select wire:model.live="sortBy" class="apis-toolbar-control">
                        ...
                        <option value="needed_asc">Date needed: earliest</option>
                        <option value="needed_desc">Date needed: latest</option>
                    </select>
                </div>
            </div>
            ...
            <div class="apis-card" x-data="{ open: false }">
                <button type="button" @click="open = !open" class="w-full text-left p-[14px_18px] flex justify-between items-start gap-3" style="cursor: pointer;">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-[7px] mb-[5px] flex-wrap">
                            ...
                            <span class="text-[11px] px-2 py-0.5 rounded font-medium" style="background: var(--blue-bg); color: var(--blue)">Recommended</span>
                        </div>
                ...
                <div x-cloak x-show="open" class="border-t p-[16px_18px]" style="border-color: var(--border)">
                    ...
                    <div class="mb-[14px]">
                        ...
                        <textarea wire:model.live="remarks.{{ $request['id'] }}" class="apis-remarks-control" placeholder="Add VP approval or rejection remarks here..."></textarea>
                    </div>
