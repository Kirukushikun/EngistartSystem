            ...
            <div class="apis-card" x-data="{ open: false }">
                ...
                <div x-cloak x-show="open" class="border-t p-[16px_18px]" style="border-color: var(--border)">
                    <div class="mb-[14px] space-y-[8px] text-[12px] text-apis-text2">
                        ...
                        <div><span class="mr-1">Justification Letter:</span><span class="text-apis-blue">{{ $request['jl'] ?? 'No attachment found' }}</span></div>
                    </div>
                    <p class="text-[12px] leading-[1.7] text-apis-text mb-[14px] border-l-2 pl-3" style="border-color: var(--border)">{{ $request['desc'] }}</p>
                    <div class="mb-[14px]">
                        <p class="text-[10px] text-apis-text2 mb-2 font-medium uppercase tracking-[0.07em]">Routing Chain</p>
                    ...
                    <div class="mb-[14px]">
                        ...
                        <div class="rounded-[12px] p-[12px]" style="background: var(--bg2); border: 0.5px solid var(--border)">
                            ...
                            @if ($request['isPendingHere'])
                                <textarea wire:model.live="remarks.{{ $request['id'] }}" class="apis-remarks-control" placeholder="Add validation remarks here..."></textarea>
                            @endif
                        </div>
                    </div>
                    <div class="flex gap-2 flex-wrap">
                        @if ($request['isPendingHere'])
                            <button type="button" wire:click="confirmApprove(@js($request['id']))" class="apis-card-button font-medium" style="background: var(--green-bg); color: var(--green); border: 0.5px solid var(--green-bd)">Approve Late Request</button>
                            <button type="button" wire:click="confirmReject(@js($request['id']))" class="apis-card-button font-medium" style="background: var(--red-bg); color: var(--red); border: 0.5px solid var(--red-bd)">Reject</button>
                        @else
