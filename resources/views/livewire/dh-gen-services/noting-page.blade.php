...
@section('sidebar')
    ...
    <a href="{{ route('dh-gen-services.noting') }}" class="flex items-center justify-between rounded-md px-3 py-2 text-sm font-medium bg-apis-bg text-apis-text" style="border: 0.5px solid var(--border2)"><span>For Noting</span><span class="text-[10px] px-1.5 py-0.5 rounded" style="background: var(--blue-bg); color: var(--blue)">{{ $this->filteredItems->count() }}</span></a>
...
<div class="p-6 overflow-y-auto h-full">
    <div class="w-full">
        ...
        @if ($this->filteredItems->isNotEmpty()) @include('partials.apis.alert', ['type' => 'warn', 'message' => $this->filteredItems->count() . ' request' . ($this->filteredItems->count() !== 1 ? 's' : '') . ' pending your notation.']) @endif
        ...
        <div class="rounded-[12px] p-[12px_14px] mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between" style="border: 0.5px solid var(--border); background: var(--bg2)">
            <div class="grid grid-cols-1 md:grid-cols-[minmax(0,1.5fr)_180px_180px] gap-3 flex-1 w-full"><div><label class="block text-[10px] text-apis-text2 mb-1 font-medium uppercase tracking-[0.07em]">Search</label><input type="text" wire:model.live.debounce.300ms="search" class="apis-toolbar-control" placeholder="Search by request ID, title, farm, or requester..."></div><div><label class="block text-[10px] text-apis-text2 mb-1 font-medium uppercase tracking-[0.07em]">Type</label><select wire:model.live="typeFilter" class="apis-toolbar-control"><option value="all">All types</option>@foreach ($this->typeOptions as $type)<option value="{{ $type }}">{{ $type }}</option>@endforeach</select></div><div><label class="block text-[10px] text-apis-text2 mb-1 font-medium uppercase tracking-[0.07em]">Sort</label><select wire:model.live="sortBy" class="apis-toolbar-control"><option value="latest">Latest submitted</option><option value="needed_asc">Date needed: earliest</option><option value="needed_desc">Date needed: latest</option></select></div></div>
        ...
        @forelse ($this->paginatedItems as $request)
            <div class="apis-card" x-data="{ open: false }">
                <button type="button" @click="open = !open" class="w-full text-left p-[14px_18px] flex justify-between items-start gap-3" style="cursor: pointer;">
                    <div class="flex-1 min-w-0"><div class="flex items-center gap-[7px] mb-[5px] flex-wrap"><span class="font-mono text-[11px] text-apis-text2 whitespace-nowrap">{{ $request['id'] }}</span><span class="text-[11px] px-2 py-0.5 rounded font-medium" style="background: var(--blue-bg); color: var(--blue)">VP Approved</span></div><p class="text-[14px] font-medium m-0 mb-[3px] overflow-hidden text-ellipsis whitespace-nowrap text-apis-text">{{ $request['title'] }}</p><p class="text-[11px] text-apis-text2 m-0">{{ $request['farm'] }} · Needed {{ $request['needed'] }} · {{ $request['days'] }}d ahead · By {{ $request['by'] }}</p></div>
                ...
                <div x-cloak x-show="open" class="border-t p-[16px_18px]" style="border-color: var(--border)">
                    <div class="mb-[14px] space-y-[8px] text-[12px] text-apis-text2"><div><span class="mr-1">Type:</span><span class="text-apis-text">{{ $request['type'] }}</span></div><div><span class="mr-1">Purpose:</span><span class="text-apis-text">{{ $request['purpose'] }}</span></div></div>
                    <p class="text-[12px] leading-[1.7] text-apis-text mb-[14px] border-l-2 pl-3" style="border-color: var(--border)">{{ $request['desc'] }}</p>
                    <div class="mb-[14px]"><p class="text-[10px] text-apis-text2 mb-2 font-medium uppercase tracking-[0.07em]">Approval Chain</p><div class="flex flex-col gap-[7px]">@foreach ($request['chain'] as $step) @php $stepStyle = match ($step['st']) { 'done' => ['bg' => 'var(--green-bg)', 'color' => 'var(--green)', 'symbol' => '✓'], 'pending' => ['bg' => 'var(--blue-bg)', 'color' => 'var(--blue)', 'symbol' => '●'], 'rejected' => ['bg' => 'var(--red-bg)', 'color' => 'var(--red)', 'symbol' => '✕'], default => ['bg' => 'var(--gray-bg)', 'color' => 'var(--text3)', 'symbol' => '○'], }; @endphp <div class="flex items-start gap-[9px]"><span class="apis-step-dot" style="background: {{ $stepStyle['bg'] }}; color: {{ $stepStyle['color'] }};">{{ $stepStyle['symbol'] }}</span><div class="flex-1 pt-[1px]"><div class="flex justify-between items-baseline gap-2"><span class="text-[12px] {{ $step['st'] === 'waiting' ? 'text-apis-text3' : 'text-apis-text' }} {{ $step['st'] === 'pending' ? 'font-medium' : 'font-normal' }}">{{ $step['role'] }} — {{ $step['action'] }}</span>@if ($step['date'])<span class="text-[11px] text-apis-text3 flex-shrink-0">{{ $step['date'] }}</span>@endif</div>@if ($step['user'])<span class="text-[11px] text-apis-text2">{{ $step['user'] }}</span>@endif</div></div> @endforeach</div></div>
                    <div class="mb-[14px]"><label class="block text-[10px] text-apis-text2 mb-2 font-medium uppercase tracking-[0.07em]">Remarks</label><textarea wire:model.live="remarks.{{ $request['id'] }}" class="apis-remarks-control" placeholder="Add notation remarks here..."></textarea></div>
                    <div class="flex gap-2 flex-wrap"><button type="button" wire:click="noteForward(@js($request['id']))" class="apis-card-button font-medium" style="background: var(--blue-bg); color: var(--blue); border: 0.5px solid var(--blue-bd)">Note &amp; Forward to ED Manager</button></div>
                </div>
            </div>
        @empty
        ...
        <div class="mt-4 rounded-[12px] p-[12px_14px] flex flex-col gap-3 md:flex-row md:items-center md:justify-between" style="border: 0.5px solid var(--border); background: var(--bg)"><p class="text-[12px] text-apis-text2 m-0">Showing {{ $this->showingFrom }}-{{ $this->showingTo }} of {{ $this->filteredItems->count() }} request{{ $this->filteredItems->count() !== 1 ? 's' : '' }}</p><div class="flex items-center gap-2"><button type="button" wire:click="previousPage" @disabled($page <= 1) class="apis-card-button" style="border: 0.5px solid var(--border2); background: var(--bg2); color: var(--text); opacity: {{ $page <= 1 ? '0.5' : '1' }};">Previous</button><span class="text-[12px] text-apis-text2 px-1">Page {{ $page }} of {{ $this->totalPages }}</span><button type="button" wire:click="nextPage" @disabled($page >= $this->totalPages) class="apis-card-button" style="border: 0.5px solid var(--border2); background: var(--bg2); color: var(--text); opacity: {{ $page >= $this->totalPages ? '0.5' : '1' }};">Next</button></div></div>
    </div>
</div>
@endsection
