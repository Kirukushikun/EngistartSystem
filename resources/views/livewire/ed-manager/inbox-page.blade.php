...
@section('sidebar')
    <a href="{{ route('ed-manager.inbox') }}" class="flex items-center justify-between rounded-md px-3 py-2 text-sm font-medium bg-apis-bg text-apis-text" style="border: 0.5px solid var(--border2)">
        <span>For Acceptance</span>
        <span class="text-[10px] px-1.5 py-0.5 rounded" style="background: var(--green-bg); color: var(--green)">{{ $this->filteredInboxItems->count() }}</span>
    </a>
    <a href="{{ route('ed-manager.change-request') }}" class="flex items-center rounded-md px-3 py-2 text-sm text-apis-text2 transition-colors hover:bg-apis-bg hover:text-apis-text">Settings Change Request</a>
@endsection
...
<div class="p-6 overflow-y-auto h-full">
    <div class="w-full">
        ...
        @if ($this->filteredInboxItems->isNotEmpty())
            @include('partials.apis.alert', ['type' => 'warn', 'message' => $this->filteredInboxItems->count() . ' request' . ($this->filteredInboxItems->count() !== 1 ? 's' : '') . ' awaiting your final acceptance.'])
        @endif
        ...
        @forelse ($this->paginatedInboxItems as $request)
            <div class="apis-card" x-data="{ open: false }">
                <button type="button" @click="open = !open" class="w-full text-left p-[14px_18px] flex justify-between items-start gap-3" style="cursor: pointer;">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-[7px] mb-[5px] flex-wrap">
                            ...
                            <span class="text-[11px] px-2 py-0.5 rounded font-medium" style="background: var(--green-bg); color: var(--green)">Awaiting Acceptance</span>
                        </div>
                ...
                <div x-cloak x-show="open" class="border-t p-[16px_18px]" style="border-color: var(--border)">
                    ...
                    <div class="mb-[14px]"><label class="block text-[10px] text-apis-text2 mb-2 font-medium uppercase tracking-[0.07em]">Remarks</label><textarea wire:model.live="remarks.{{ $request['id'] }}" class="apis-remarks-control" placeholder="Add acceptance or return remarks here..."></textarea></div>
                    <div class="flex gap-2 flex-wrap"><button type="button" wire:click="accept(@js($request['id']))" class="apis-card-button font-medium" style="background: var(--green-bg); color: var(--green); border: 0.5px solid var(--green-bd)">Accept</button><button type="button" wire:click="returnRequest(@js($request['id']))" class="apis-card-button font-medium" style="background: var(--red-bg); color: var(--red); border: 0.5px solid var(--red-bd)">Return</button></div>
                </div>
            </div>
        @empty
        ...
        <div class="mt-4 rounded-[12px] p-[12px_14px] flex flex-col gap-3 md:flex-row md:items-center md:justify-between" style="border: 0.5px solid var(--border); background: var(--bg)">
            <p class="text-[12px] text-apis-text2 m-0">Showing {{ $this->showingFrom }}-{{ $this->showingTo }} of {{ $this->filteredInboxItems->count() }} request{{ $this->filteredInboxItems->count() !== 1 ? 's' : '' }}</p>
